...
const controller = () => {
  ...
  const addSubMenus = (
    ...
    fetch(`${import.meta.env.VITE_BACKEND_URL}/cyclemart/menus/${menuId}`, {
      method: "PUT",
      headers: {
  ...
  const deletCategoryMenu = (id, userToken, _alart, update, setUpdate) => {
    fetch(`${import.meta.env.VITE_BACKEND_URL}/cyclemart/menus/${id}`, {
      method: "DELETE",
      headers: {
  ...
  function deleteSubCategoryMenu(menus, update, setUpdate) {
    fetch(`${import.meta.env.VITE_BACKEND_URL}/cyclemart/menus`, {
      method: "PUT",
      headers: {
        "content-type": "application/json",
      },
